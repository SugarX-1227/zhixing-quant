...
- 「超额」先按日平均再跨日平均（每个交易日权重相同），t 值也按日算：
  同一天的多个命中高度相关，不能当独立样本。同时报「每笔超额」（每个命中
  权重相同）和中位数：命中扎堆的日子会让两者差很多——B2 按天 -0.51%、
  每笔 +0.30%、中位 -0.82%，典型的一笔跑输，少数集中爆发的日子把平均拉正。
  三个数要一起看，只看一个会得出相反的结论。
...
def stat(ev: pd.DataFrame, mask: pd.Series, col: str, label: str) -> dict:
    ...
    return {"": label, "事件数": len(s), "天数": n, "原始收益": s[col].mean(),
            "超额": daily.mean(), "t值": t, "每笔超额": s["x_" + col].mean(),
            "超额中位": s["x_" + col].median(), "跑赢比例": (s["x_" + col] > 0).mean()}
