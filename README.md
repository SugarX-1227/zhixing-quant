...
| 因子有效性 | 无 | 「因子」页给 IC / ICIR / 分层收益 / 单调性 / **方向诊断** |
| 因子权重 | 手写预设（拍脑袋） | 「因子」页按实测 IC 自动生成，可直接粘回配置 |
| 出场规则调试 | 改一条看总收益，全靠感觉 | 「回测」页出场规则消融，逐条关掉看边际贡献 |
| 参数校准 | 一次一组参数、一个区间、一个数字 | 「校准」页网格扫描（样本内选/样本外验）+ 滚动前进 + 参数平坦度 |
| 配置写错 | 静默走默认值，界面看不出来 | 加载时校验：重复键/枚举值/哑开关/未注册因子一律拦下 |
