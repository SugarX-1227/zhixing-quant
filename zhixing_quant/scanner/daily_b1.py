...
def scan_daily(cfg: dict, end_date: str = None, limit_universe: int = None) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    for _, stock in universe.iterrows():
        ...
        try:
            ...
            if len(daily) < max(cfg["b1"]["yellow_ma_windows"]):
                continue
            enriched = add_b1_indicators(daily, cfg)
            latest = enriched.iloc[-1]
            chart_data[code] = enriched.tail(int(cfg["report"]["lookback_bars"]))
            if not bool(latest["sig_b1"]):
                continue
            row = {
                ...
                "reason": (
                    f"J值({float(latest['kdj_j']):.1f})<{cfg['b1']['j_threshold']}"
                    f" + 站上知行多空线"
                    f" + 短期趋势线确认"
                    f" + 涨跌幅{float(latest['pct_chg']):.2f}%在±4%内"
                ),
            }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run daily B1 stock scanner.")
    ...
    path = render_daily_report(report_date, candidates, chart_data, cfg, strategy="b1")
    print(f"Report: {path}")
